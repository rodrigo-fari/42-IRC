...
void PrivMsgCommand::execute(int fd, const MessagePayload& payload, ReplyCollector &replies)
{
	...
	{
		replies.error(ErrorReply(ERR_NEEDMOREPARAMS, target, "PRIVMSG", "", "Not enough parameters"));
		return;
	}
	...
	if (messageTarget[0] == '#')
	{
		...
		if (!channel)
		{
			replies.error(ErrorReply(ERR_NOSUCHCHANNEL, target, "", messageTarget, "No such channel"));
			return;
		}
