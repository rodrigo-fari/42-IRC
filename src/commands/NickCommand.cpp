...
/*   Updated: 2026/02/23 23:48:51 by rerodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
NickCommand::NickCommand(
	UserRepository& ur, ChannelRepository& cr, ClientStateRepository& csr, const std::string& srv
)
...
void NickCommand::execute(int fd, const MessagePayload& payload, ReplyCollector& replies) {
	...
	if (!wasRegistered && state.isRegistered)
		replies.raw(
			":" + serverName + " 001 " + state.nickname + " :[Welcome] to  IRC 42 " + state.nickname
		);
}
