...
void ModeCommand::execute(int fd, const MessagePayload& payload, ReplyCollector& replies) {
	...
	if (payload.params.size() == 1) {
		...
		if (channel->inviteOnlyPolicy)
			modes += "i";
		if (channel->topicLockPolicy)
			modes += "t";
	...
	for (size_t i = 0; i < parsedModes.size(); ++i) {
		...
		const bool needsArg =
			((mode == 'k' && sign == '+') || (mode == 'l' && sign == '+') || (mode == 'o'));
		...
		if (argIndex >= payload.params.size()) {
			replies.error(ErrorReply(
				ERR_NEEDMOREPARAMS,
				target,
				"MODE " + std::string(1, sign) + std::string(1, mode),
				"",
	...
	for (size_t i = 0; i < parsedModes.size(); ++i) {
		...
		if (valid) {
			...
			if (errno != 0 || end == parsedArgs[i].c_str() || *end != '\0' || value <= 0 ||
				value > INT_MAX)
				valid = false;
		}
		...
		if (!valid) {
			replies.error(
				ErrorReply(ERR_INVALIDMODEPARAM, target, channelName, "l", "Invalid mode parameter")
			);
			parsedArgs[i].clear();
		}
	}
