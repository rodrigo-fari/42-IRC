...
DispatchResult Dispatcher::dispatch(int fd, const MessagePayload &payload)
{
	...
	try
	{
		...
		if (cmd == "PRIVMSG")
		{
			PrivMsgCommand privMsgCmd(userRepository, channelRepository, clientStateRepository, serverName);
		...
		if (cmd == "PASS")
		{
			PassCommand passCmd(userRepository, channelRepository, clientStateRepository, serverName, serverPassword);
